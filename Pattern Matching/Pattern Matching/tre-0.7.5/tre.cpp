...
int main()
{
  ...
  regex_t a;
  string regex = "^test\\s([[:print:]]*)$"; //the regular expression
  ...
  hr = regcomp(&a, regex.c_str(), REG_EXTENDED | REG_ICASE | REG_NEWLINE);
  ...
  if (hr) { //error
    ...
    string testStr = "taste a variable"; //string to test against regex
    ...
    if (hr) { //error
      ...
      for (int i = 1; i < match.nmatch; i++) { //pmatch[0] is just the start and end for the whole string
